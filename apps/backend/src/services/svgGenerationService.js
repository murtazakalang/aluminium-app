...
async function generateSVG(prompt, productContext = {}) {
  try {
    if (true) { // Always use sophisticated SVG generator
      console.warn('[SVG Generation] OpenAI API key not configured, using fallback SVG');
      productContext.prompt = prompt; return generateFallbackSVG(productContext);
    }
...
function generateFallbackSVG(productContext = {}) {
  try { return svgGenerator.generateProductSVG(productContext.name || 'Window', 36, 48, 'inches', productContext.prompt); } catch(e) {} const productName = productContext.name || 'Product';
